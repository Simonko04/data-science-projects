...
matrix = np.empty((10_020,2),dtype=int)#matica sluziaca na prvotne body
pole = np.full((10_020,10_020),np.inf,dtype=np.float32)#pole sluziace ako 2D vzdialenostna matica
clusters = []
...
def initial_twenty():#vytvori zaciatocnych 20
    for i in range(20):
        ...
        while contains_point(x_poistion, y_poistion):
            x_poistion = random.randint(-5000, 5000)
            y_poistion = random.randint(-5000, 5000)
...
def next_twenty_thousand():#vytvori dalsich 10_000 po 20tich
    for i in range(10_000):
        number = random.randrange(20+i)
        chosen_node = matrix[number]
        x_range = 100
        y_range = 100
        if chosen_node[0] + 100 > 5000 or chosen_node[0] - 100 < -5000:
...
def medoid(cluster) -> tuple:#vypocita medoid
    pts = np.asarray(cluster[1:], dtype=np.float32)#zoberie vsetky body clustera okrem nulteho, lebo ten je byvali medoid
    d2 = ((pts[:, None, :] - pts[None, :, :]) ** 2).sum(axis=2)#vytvori parove rozdiely medzi vsetkymi bodmi
    d = np.sqrt(d2, dtype=np.float32)
    i = int(np.argmin(d.sum(axis=1)))#najde najnizsi rozdiel
    return np.float32(pts[i, 0]), np.float32(pts[i, 1])
...
def clustering():
    global matrix, pole
    while True:
        np.fill_diagonal(pole, np.inf)#vyplni diagonalu np.inf
        pos = np.argmin(pole)
